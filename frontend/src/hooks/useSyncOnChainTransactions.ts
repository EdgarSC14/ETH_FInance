...
import { CONTRACT_ADDRESSES, DEMO_TRANSACTIONS } from "@/lib/constants";
...
/**
 * Loads vault / goal / payment activity from chain logs for the connected wallet.
 */
export function useSyncOnChainTransactions() {
    ...
    try {
      ...
      if (cfg.paymentRouter?.trim()) {
        ...
        for (const payment of rawPayments) {
          const executed = await router.queryFilter(
  ...
  }, [isConnected, provider, address, chainId, setTransactions, setOnChainTransactionsLoading]);

  useEffect(() => {
    if (!isConnected) return;
    refresh();
  }, [isConnected, refresh, transactionsRefreshKey]);
